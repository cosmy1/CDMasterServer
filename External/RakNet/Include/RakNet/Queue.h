...
namespace BasicDataStructures
{
	...
	public:
		...
		queue_type& operator[] ( unsigned long position ); // Not a normal thing you do with a queue but can be used for efficiency
		void del( unsigned long position ); // Not a normal thing you do with a queue but can be used for efficiency
	...
	private:
		queue_type* array;
		unsigned long head;  // Array index for the head of the queue
		unsigned long tail; // Array index for the tail of the queue
		unsigned long allocation_size;
	};
	
	
	template <class queue_type>
	inline const unsigned long Queue<queue_type>::size( void ) const
	{
		if ( head <= tail )
			return tail -head;
		else
	...
	void Queue<queue_type>::pushAtHead( const queue_type& input )
	{
		...
		if ( tail == head )
		{
			...
			for ( unsigned long counter = 0L; counter < allocation_size; ++counter )
	...
	void Queue<queue_type>::compress ( void )
	{
		...
		new_array = new queue_type [size()+1];
		 
		array = new queue_type [original_copy.size()];
		
		for (unsigned long counter=0L; counter < size(); ++counter)
		array[counter] = original_copy.array[(head + counter)%(allocation_size)];
		
		allocation_size=original_copy.size();
	...
	template <class queue_type>
	inline queue_type& Queue<queue_type>::operator[] ( unsigned long position )
	{
#ifdef _DEBUG
	...
	template <class queue_type>
	void Queue<queue_type>::del( unsigned long position )
	{
#ifdef _DEBUG
		assert( position < size() );
		assert( head != tail );
